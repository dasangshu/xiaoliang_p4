...
Application::Application() {
    ...
    // 安装串口驱动
    uart_driver_install(UART_NUM_1, 1024 * 1, 1024 * 1, 0, NULL, 0);
    // 配置串口引脚
    uart_set_pin(UART_NUM_1, UART_PIN_NO_CHANGE, 49, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
    // 应用串口配置
    uart_param_config(UART_NUM_1, &uart_config);
     ...
     xTaskCreate([](void* arg) {
        ...
        while (true) {
            ...
            if (recv_length)
            {
                ESP_LOGI(TAG, "recv_length:%u ", recv_length);
                int len = uart_read_bytes(UART_NUM_1, buffer, recv_length, 10 / portTICK_PERIOD_MS);
                for (size_t i = 0; i < len; i++)
                {
                    printf("%x ", buffer[i]);
                }
                if (buffer[0] == 0xAA && buffer[2] == 0x00 && buffer[1]== 0x55 && buffer[3]== 0x55)
                {
                    // ToggleChatState();
                    std::string wake_word="小亮同学";
    ...
    }, "uart1_receive_task", 4096, this, 2, nullptr);
...
void Application::SetDeviceState(DeviceState state) {
    ...
    switch (state) {
        ...
        case kDeviceStateConnecting:
            ...
            display->SetEmotion("xiaoliang.mjpeg");
            display->SetChatMessage("system", "");
            break;
