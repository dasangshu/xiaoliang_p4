...
SpiLcdDisplay::SpiLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    port_cfg.task_priority = 4;       // 提高LVGL任务优先级
    port_cfg.timer_period_ms = 16;    // 16ms = 62.5 FPS，支持更高帧率
...
void LcdDisplay::SetupUI() {
    ...
    // 不使用flex布局，直接使用绝对定位
    lv_obj_set_flex_flow(content_, LV_FLEX_FLOW_COLUMN); // 保持垂直布局用于文字部分
    lv_obj_set_flex_align(content_, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER); 
    
    // 创建AVI视频画面，固定在左下角
    avi_image = lv_canvas_create(screen); // 直接创建在screen上，而不是content内
    // 设置AVI画面大小和位置 - 放在屏幕左下角，留一些边距
    int avi_width = LV_HOR_RES / 3;  // 屏幕宽度的1/3
    int avi_height = LV_VER_RES / 3; // 屏幕高度的1/3
    lv_obj_set_size(avi_image, avi_width, avi_height);
    // 定位在左下角，留10像素边距
    lv_obj_align(avi_image, LV_ALIGN_BOTTOM_LEFT, 10, -10);
    // 设置较高的z-index确保在其他元素之上
    lv_obj_move_to_index(avi_image, -1);
    
    // 为了避免文字与视频重叠，调整content的右边距
    lv_obj_set_style_pad_left(content_, avi_width + 20, 0); // 为AVI画面预留空间
    ...
    // 调整文字区域宽度，为AVI画面留出空间
    lv_obj_set_width(chat_message_label_, (LV_HOR_RES - avi_width - 30) * 0.9); // 减去AVI宽度和边距
...
void LcdDisplay::changeEyeStyle()
{
    ...
    if (avi_image)
    {
        ...
        // 切换到下一个眼球视频
        current_eye_index = (current_eye_index + 1) % 10;
        mjpeg_player_port_play_file(eye_videos[current_eye_index]);
    }
}
...
void LcdDisplay::SetupUI() {
    ...
    avi_image = lv_canvas_create(screen); // 直接创建在screen上，而不是content内
    // 设置AVI画面大小和位置 - 放在屏幕左下角，留一些边距
    int avi_width = LV_HOR_RES / 2;  // 屏幕宽度的1/3
    int avi_height = LV_VER_RES / 2; // 屏幕高度的1/3
    // lv_obj_set_size(avi_image, avi_width, avi_height);
    // 定位在左下角，留10像素边距
    lv_obj_align(avi_image, LV_ALIGN_BOTTOM_LEFT, 0, 0);
    // 设置较高的z-index确保在其他元素之上
    lv_obj_move_to_index(avi_image, -1);
...
void LcdDisplay::SetFaceImage(uint8_t *frame_buffer, int width, int height)
{
    ...
    // 立即标记对象为无效，强制刷新显示
    lv_obj_invalidate(avi_image);
    
    // 可选：如果性能仍有问题，可以尝试立即处理LVGL任务
    // lv_timer_handler();  // 谨慎使用，可能导致死锁
}
